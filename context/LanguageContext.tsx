...
export const strings = {
  es: {
    ...
    playlistsCount_one: 'lista',
    playlistsCount_other: 'listas',
    yourCustomPlaylists: 'Tus listas de reproducción personalizadas',
    songCount_one: 'canción',
    songCount_other: 'canciones',
    deletePlaylist: 'Eliminar Lista',
    deletePlaylistTitle: 'Eliminar lista',
    deletePlaylistConfirm: '¿Estás seguro de que quieres eliminar esta lista de reproducción?',
    delete: 'Eliminar',
    ...
    doctrinePurposeBody: 'Nuestra finalidad objetiva es ver establecida en nuestra Iglesia la santidad, la fraternidad, la honestidad y la justicia en todos nuestros acts, y que todos los componentes estemos unidos formando un cuerpo, como es uno el cuerpo del Señor Jesús (Sal. 133:1-3; Juan 17:21,23; 1 Cor. 12:25,27).\n\nExiste otra finalidad y es uno de los principios fundamentales de nuestra Iglesia: Avivar la predicación del santo evangelio de nuestro Señor Jesucristo (Mar. 16:15; Juan 17:20; 1 Cor. 9:16,18).\n\nEl Señor en su plan divino fundó su Iglesia con el fin de congregar en ella los que han de ser salvos (Hech. 2:47).',
    nowPlaying: 'REPRODUCIENDO',
    sectionHimnos: 'Himnos',
    sectionEstribillos: 'Estribillos y Coritos',
    indiceEstribillosTitle: 'ÍNDICE ALFABÉTICO DE ESTRIBILLOS Y CORITOS',
    indiceHimnosTitle: 'ÍNDICE ALFABÉTICO DE HIMNOS',
    searchResultsCount_one: 'resultado',
    searchResultsCount_other: 'resultados',
    footerQuoteText: '"Cantad alegres a Dios, habitantes de toda la tierra."',
    footerQuoteAuthor: 'SALMO 100:1',
  },
  en: {
    ...
    playlistsCount_one: 'playlist',
    playlistsCount_other: 'playlists',
    yourCustomPlaylists: 'Your custom playlists',
    songCount_one: 'song',
    songCount_other: 'songs',
    ...
    sectionHimnos: 'Hymns',
    sectionEstribillos: 'Choruses & Refrains',
    indiceEstribillosTitle: 'ALPHABETICAL INDEX OF CHORUSES & REFRAINS',
    indiceHimnosTitle: 'ALPHABETICAL INDEX OF HYMNS',
    searchResultsCount_one: 'result',
    searchResultsCount_other: 'results',
    footerQuoteText: '"Make a joyful noise unto God, all ye lands."',
    footerQuoteAuthor: 'PSALM 100:1',
  },
};
...
export const translateCategory = (cat: string, lang: Language): string => {
  ...
  return categoryTranslations[trimmed] || trimmed;
};

export const translateCategories = (categoriesStr: string, lang: Language): string => {
  if (!categoriesStr) return '';
  const parts = categoriesStr.split('•').map(c => c.trim()).filter(Boolean);
  if (lang === 'es') return parts.join('  ·  ');
  return parts.map(c => translateCategory(c, 'en')).join('  ·  ');
};
